#!/usr/bin/bash
#SBATCH --job-name=sicithkpost
#SBATCH --output=logs/exp1_icethick_rerun_postprocess.%j.out
#SBATCH --error=logs/exp1_icethick_rerun_postprocess.%j.err
...
python -m src.models.diagnostics --config "${selector}" --overwrite

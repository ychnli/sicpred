python3 -m src.models.train --config src/experiment_configs/exp3_obs/obs_input2.py --members 20
python3 -m src.models.evaluate --config src/experiment_configs/exp3_obs/obs_input2.py

python3 -m src.models.train --config src/experiment_configs/exp3_obs/obs_input4.py --members 20
python3 -m src.models.evaluate --config src/experiment_configs/exp3_obs/obs_input4.py
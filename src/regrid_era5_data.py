...
"""
def regrid_var(var_name, output_grid=config.SPS_GRID, overwrite=False):
    if not os.path.exists(f'{config.DATA_DIRECTORY}/ERA5/{var_name}.nc'):
        raise FileNotFoundError(f"{file_path} does not exist")
    
    print(f'Regridding {var_name}')
    if os.path.exists(f'{config.DATA_DIRECTORY}/ERA5/{var_name}_SPS.nc'):
    ...
    ds_to_regrid = xr.open_dataset(f'{config.DATA_DIRECTORY}/ERA5/{var_name}.nc')
    ...
    weight_file = f'{config.DATA_DIRECTORY}/ERA5/ERA5_to_SPS_bilinear_regridding_weights.nc'
    ...
    ds_regridded.to_netcdf(f'{config.DATA_DIRECTORY}/ERA5/{var_name}_SPS.nc')
    print('Done!')
